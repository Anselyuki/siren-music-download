...
└── lib/
    ...
    ├── api.ts                         # 主 Tauri command bridge（含 tag registry bridge）
...
前后端共享类型（`src/lib/types.ts`）中新增：

- `TagEntry { dimension: string; values: string[] }` — 已本地化的 tag 条目，附加在 `Album`、`AlbumDetail`、`SongEntry`、`SongDetail` 上
- `TagDimension { key: string; label: string }` — tag 维度摘要，用于维度列表展示
- `TagGroup { dimensionKey: string; value: string; albums: Album[] }` — 按 tag 值聚合的专辑分组

`Album`、`AlbumDetail`、`SongEntry`、`SongDetail` 均已新增 `tags: TagEntry[]` 字段（`#[serde(default)]`，无 tag 时为空数组）。

`LibrarySearchHitField` 新增 `'tagValues'` 枚举值，表达搜索命中 tag 维度值。

Bridge 函数（`src/lib/api.ts`）：

- `getTagDimensions(): Promise<TagDimension[]>` — 获取当前可用的 tag 维度列表
- `getAlbumsByTagDimension(dimensionKey: string): Promise<TagGroup[]>` — 按指定维度聚合专辑

两个函数均为只读调用，locale 由后端从用户偏好自动读取。
