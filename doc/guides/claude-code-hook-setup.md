...
本文档说明如何把仓库内 [scripts/claude-hooks/](../../scripts/claude-hooks/) 的格式化 hook 安装到你本地的 `.claude/hooks/`，并在 Claude Code 本地配置中启用。
...
项目根下的 `.claude/` 默认被 `.gitignore` 忽略，因此推荐把这些脚本复制到你自己的 `.claude/hooks/` 中，再通过 `.claude/settings.local.json` 启用。
